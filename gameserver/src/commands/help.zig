...
pub fn handle(session: *Session, _: []const u8, allocator: Allocator) !void {
    ...
    try commandhandler.sendMessage(session, "/stop to disconnect your session; /kick to force a client-side logout", allocator);
    try commandhandler.sendMessage(session, "/mail to push a test in-game mail with small gifts", allocator);
}
